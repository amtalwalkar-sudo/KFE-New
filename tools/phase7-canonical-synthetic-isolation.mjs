...
try{
  ...
  async function createVehicle(registration){
    if(await page.getByRole('button',{name:/Create Vehicle/}).count()===0) { await page.locator('.admin-list').waitFor({state:'visible',timeout:30000}); await page.locator('.admin-item').filter({hasText:/^Vehicle$/}).click() }
