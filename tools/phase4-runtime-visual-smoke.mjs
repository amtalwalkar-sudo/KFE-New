...
try{
 ...
 // 4C GPS
 const gps=page.locator('button.header-gps');assert(await gps.count()===1,'GPS control missing');await wait(async()=>await gps.getAttribute('aria-label')==='GPS connected');assert(!(await gps.innerText()).trim(),'GPS control is not icon-only')
 // 4D themes
 await page.evaluate(()=>{localStorage.setItem('kfe.visual.theme.mode','light');location.reload()});await page.locator('.cockpit').waitFor({state:'visible'});assert(await page.locator('html').getAttribute('data-kfe-theme')==='day','light theme failed')
 const light=await page.evaluate(()=>getComputedStyle(document.documentElement).getPropertyValue('--kfe-ui-bg').trim())
 await page.evaluate(()=>{localStorage.setItem('kfe.visual.theme.mode','dark');location.reload()});await page.locator('.cockpit').waitFor({state:'visible'});assert(await page.locator('html').getAttribute('data-kfe-theme')==='night','dark theme failed')
 ...
 await page.evaluate(()=>{localStorage.setItem('kfe.visual.theme.mode','auto');localStorage.setItem('kfe.visual.theme.schedule',JSON.stringify({dayStart:'00:00',nightStart:'23:59'}));location.reload()});await page.locator('.cockpit').waitFor({state:'visible'});assert(['day','night'].includes(await page.locator('html').getAttribute('data-kfe-theme')),'auto theme invalid')
