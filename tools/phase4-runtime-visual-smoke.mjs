...
try{
 ...
 const route=async(path,selector,label)=>{const res=await page.goto(base+path,{waitUntil:'domcontentloaded',timeout:30000});assert(res?.ok(),label+' response failed');await Promise.race([page.locator(selector).waitFor({state:'visible',timeout:20000}),page.locator('.kfe-runtime-error').waitFor({state:'visible',timeout:20000}).then(async()=>{throw new Error(label+' startup/runtime error: '+(await page.locator('body').innerText()).slice(0,1000))})]);await healthy(label)}
 // 4A shell/routes
