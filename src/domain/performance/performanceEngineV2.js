...
export function derivePerformance(s, r, p = previousRange(r)) {
  ...
  const authoritativeBreakEven = deriveAuthoritativeBreakEven({ breakEvenInputs: s.breakEvenInputs, range: breakEvenRange, loanScheduledObligation: breakEvenLoan.ok ? breakEvenLoan.scheduled : NaN, renewalProvision: breakEvenRenewal, fuelCostPerKm: breakEvenFuelModel.rollingCostPerKm, vehicleKm: breakEvenVehicleKm })
  ...
  const wd = days(r.from, r.to), elapsed = wd, activeDays = new Set(a.tr.map(x => istDateKey(d(x.tripEndAt || x.tripStartAt))).filter(Boolean)).size, prevActive = new Set(q.tr.map(x => istDateKey(d(x.tripEndAt || x.tripStartAt))).filter(Boolean)).size
  ...
  return {
    ...
    breakEvenInputs: { maintenanceProvisionPerKm: maintenanceRate, fixedCosts: authoritativeBreakEven.fixedCosts, fuelCostPerKm: breakEvenFuelModel.rollingCostPerKm }, pace: { currentRevenuePerFinancialDay: perDay, requiredRevenuePerFinancialDay: NaN, paceVariance: NaN }, trips: a.tr, shifts: a.sh, fuelLogs: a.fu, maintenance: a.ma,
...
export function layerRows(card, layer, m) {
  ...
  if (card === 'cost') return layer === 0 ? [r('Operating cost', money(m.runningCost)), r('Fuel', money(m.fuelCost)), r('Maintenance', money(m.actualMaintenance)), r('Toll', money(m.toll)), r('Parking', money(m.parking))] : layer === 1 ? [r('Fuel quantity', num(m.fuelQty)), r('Cost / vehicle KM', money(m.costPerKm)), r('Fuel cost / KM', money(m.fuelCostPerKm))] : layer === 2 ? [r('Current cost', money(m.runningCost)), r('Previous cost', money(m.previous.cost))] : layer === 3 ? [r('Monthly break-even revenue', money(m.monthlyBreakEvenRevenue)), r('Revenue', money(m.revenue)), r('Gap', money(m.revenue - m.monthlyBreakEvenRevenue))] : m.shifts.map(s => r('Shift', `KM ${num(n(s.endOdometer) - n(s.startOdometer))}`))
