...
export function derivePerformance(s, r, p = previousRange(r)) {
  ...
  const asOfFuelLogs = F.filter(x => {
    const capturedAt = d(x.capturedAt || x.createdAt)
    return capturedAt && capturedAt <= r.to
  })
