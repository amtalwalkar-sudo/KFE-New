...
export const ShiftTripRepository = {
  ...
  async completeShift(data) {
    const db = await initializeActiveStorage(); return new Promise((resolve, reject) => {
  ...
  async updateTripLocationPlaceName(id, eventType, placeName) {
    ...
    const db = await initializeActiveStorage(); const now = new Date().toISOString()
    return new Promise((resolve, reject) => {
