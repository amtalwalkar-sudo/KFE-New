...
const adminSection=ref('records'),selected=ref('vehicle'),settingsSelected=ref('backup'),records=ref([]),editing=ref(null),draft=ref({}),formOpen=ref(false),loading=ref(false),error=ref(''),notice=ref('')
...
function chooseSetting(key){settingsSelected.value=key}
...
<section class="admin-page" aria-label="Admin">
  ...
  <nav class="admin-nav" aria-label="Admin menu">
    <button :class="{active:adminSection==='records'}" @click="openRecords"><span>▦</span><span><strong>Records</strong><small>Source data</small></span></button>
    <button :class="{active:adminSection==='settings'}" @click="openSettings"><span>⚙</span><span><strong>Settings</strong><small>App controls</small></span></button>
  </nav>
  ...
  <template v-else>
    <section class="settings-layout">
      <aside class="settings-menu" aria-label="Settings menu">
        ...
        <button v-for="item in settingsMenu" :key="item.key" :class="{active:settingsSelected===item.key}" @click="chooseSetting(item.key)">
          <span class="settings-icon">{{item.icon}}</span><span><strong>{{item.title}}</strong><small>{{item.subtitle}}</small></span><b>›</b>
        </button>
      </aside>
...
<style scoped>
.admin-page{width:min(100%,980px);margin:0 auto;padding:24px 16px 34px;background:var(--kfe-ui-bg,#f5f7fb);color:var(--kfe-ui-text,#101828);box-sizing:border-box}.eyebrow,.section-label{font-size:.62rem;font-weight:900;letter-spacing:.13em;color:var(--kfe-muted-text,#667085)}.admin-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:16px}.admin-head h1{margin:5px 0 4px;font-size:clamp(1.55rem,6vw,2rem);letter-spacing:-.04em}.admin-head p{max-width:650px;margin:0;color:var(--kfe-muted-text,#667085);font-size:.76rem;line-height:1.5}.admin-badge{display:flex;align-items:center;gap:6px;padding:8px 11px;border:1px solid var(--kfe-ui-border,#e4e7ec);border-radius:999px;background:var(--kfe-ui-surface,#fff);font-size:.61rem;font-weight:850;white-space:nowrap}.admin-badge span{color:var(--kfe-success,#079455);font-size:.7rem}.admin-nav{display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:5px;margin-bottom:14px;border:1px solid var(--kfe-ui-border,#e4e7ec);border-radius:15px;background:var(--kfe-ui-surface,#fff)}.admin-nav button{min-height:50px;display:flex;align-items:center;justify-content:center;gap:9px;border:0;border-radius:11px;background:transparent;color:var(--kfe-muted-text,#667085);cursor:pointer}.admin-nav button>span:first-child{font-size:1rem}.admin-nav button>span:last-child{display:grid;text-align:left;gap:2px}.admin-nav strong{font-size:.72rem}.admin-nav small{font-size:.56rem}.admin-nav button.active{background:var(--kfe-accent-soft,#eff6ff);color:var(--kfe-ui-accent,#2563eb)}.group-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px}.group-card{min-height:104px;display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:10px;padding:14px;text-align:left;border:1px solid var(--kfe-ui-border,#e4e7ec);border-radius:17px;background:var(--kfe-ui-surface,#fff);color:var(--kfe-ui-text,#101828);box-shadow:0 5px 18px rgba(16,24,40,.035);cursor:pointer}.group-card.active{border-color:color-mix(in srgb,var(--kfe-ui-accent,#2563eb) 34%,var(--kfe-ui-border,#e4e7ec));background:var(--kfe-accent-soft,#eff6ff);box-shadow:0 7px 22px rgba(37,99,235,.08)}.group-icon{width:36px;height:36px;display:grid;place-items:center;border-radius:11px;background:var(--kfe-ui-bg,#f5f7fb);color:var(--kfe-ui-accent,#2563eb);font-weight:950}.group-copy{display:grid;gap:4px}.group-copy strong{font-size:.82rem}.group-copy small{font-size:.61rem;line-height:1.35;color:var(--kfe-muted-text,#667085)}.group-card>b{font-size:1.15rem;color:var(--kfe-muted-text,#667085)}.workspace,.settings-menu,.settings-workspace,.derived,.boundary{border:1px solid var(--kfe-ui-border,#e4e7ec);border-radius:18px;background:var(--kfe-ui-surface,#fff);box-shadow:0 6px 22px rgba(16,24,40,.035)}.workspace{padding:15px}.workspace-head{display:flex;justify-content:space-between;align-items:center;gap:10px}.workspace-head h2,.settings-menu h2,.settings-heading h2,.derived h2{margin:3px 0;font-size:1.08rem;letter-spacing:-.02em}.primary{min-height:42px;display:inline-flex;align-items:center;gap:5px;padding:0 12px;border:0;border-radius:11px;background:var(--kfe-ui-accent,#2563eb);color:#fff;font-size:.67rem;font-weight:900;cursor:pointer;box-shadow:0 5px 12px rgba(37,99,235,.16)}.primary span{font-size:1rem}.record-tabs{display:flex;gap:6px;overflow:auto;margin:13px -2px 4px;padding:0 2px 9px;border-bottom:1px solid var(--kfe-ui-border,#e4e7ec)}.record-tabs button{min-height:37px;padding:0 10px;border:1px solid transparent;border-radius:10px;background:var(--kfe-ui-bg,#f5f7fb);color:var(--kfe-muted-text,#667085);font-size:.62rem;font-weight:800;white-space:nowrap;cursor:pointer}.record-tabs button.active{background:var(--kfe-accent-soft,#eff6ff);border-color:color-mix(in srgb,var(--kfe-ui-accent,#2563eb) 18%,transparent);color:var(--kfe-ui-accent,#2563eb)}.message{margin:10px 0;padding:10px 12px;border-radius:10px;font-size:.7rem}.message.error{background:#fef3f2;color:#b42318}.message.notice{background:#ecfdf3;color:#067647}.record-list{display:grid;gap:8px;margin-top:10px}.record{display:flex;justify-content:space-between;align-items:flex-start;gap:14px;padding:13px;border:1px solid var(--kfe-ui-border,#e4e7ec);border-radius:13px}.record-main{min-width:0}.record-title{display:flex;align-items:center;gap:7px;flex-wrap:wrap}.record-title strong{font-size:.77rem}.record-status{padding:3px 6px;border-radius:999px;background:var(--kfe-ui-bg,#f5f7fb);color:var(--kfe-muted-text,#667085);font-size:.53rem;font-weight:800}.record-main>small{display:block;margin-top:3px;color:var(--kfe-muted-text,#667085);font-size:.59rem}.chips{display:flex;gap:5px;flex-wrap:wrap;margin-top:8px}.chips span{max-width:100%;padding:4px 7px;border-radius:7px;background:var(--kfe-ui-bg,#f5f7fb);color:var(--kfe-muted-text,#667085);font-size:.59rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.actions{display:flex;gap:6px;flex-shrink:0}.actions button{min-height:35px;padding:0 10px;border:1px solid var(--kfe-ui-border,#e4e7ec);border-radius:9px;background:var(--kfe-ui-surface,#fff);color:var(--kfe-ui-text,#101828);font-size:.62rem;font-weight:800;cursor:pointer}.actions button.delete{color:var(--kfe-danger,#d92d20)}.empty-state{min-height:190px;display:flex;align-items:center;justify-content:center;gap:7px;flex-direction:column;text-align:center;color:var(--kfe-muted-text,#667085)}.empty-state strong{color:var(--kfe-ui-text,#101828);font-size:.82rem}.empty-state span:not(.spinner){max-width:320px;font-size:.66rem;line-height:1.4}.empty-icon{width:42px;height:42px;display:grid;place-items:center;border-radius:13px;background:var(--kfe-accent-soft,#eff6ff);color:var(--kfe-ui-accent,#2563eb);font-size:1.3rem;font-weight:900}.empty-state .primary{margin-top:5px}.spinner{width:22px;height:22px;border:3px solid var(--kfe-ui-border,#e4e7ec);border-top-color:var(--kfe-ui-accent,#2563eb);border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.derived{padding:15px;margin-top:12px;box-shadow:none}.derived p{margin:7px 0 0;color:var(--kfe-muted-text,#667085);font-size:.67rem;line-height:1.5}.settings-layout{display:grid;grid-template-columns:260px 1fr;gap:12px}.settings-menu{padding:15px;box-shadow:none}.settings-menu h2{margin-bottom:13px}.settings-menu button{width:100%;display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:9px;padding:11px 9px;text-align:left;border:1px solid transparent;border-radius:12px;background:transparent;color:var(--kfe-ui-text,#101828);cursor:pointer}.settings-menu button.active{border-color:color-mix(in srgb,var(--kfe-ui-accent,#2563eb) 18%,transparent);background:var(--kfe-accent-soft,#eff6ff);color:var(--kfe-ui-accent,#2563eb)}.settings-menu button>span:nth-child(2){display:grid;gap:3px}.settings-menu strong{font-size:.69rem}.settings-menu small{font-size:.57rem;line-height:1.35;color:var(--kfe-muted-text,#667085)}.settings-menu b{color:var(--kfe-muted-text,#667085)}.settings-icon{width:32px;height:32px;display:grid;place-items:center;border-radius:9px;background:var(--kfe-ui-bg,#f5f7fb);font-weight:900}.settings-workspace{padding:15px;box-shadow:none}.settings-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:13px}.settings-heading p{margin:5px 0 0;color:var(--kfe-muted-text,#667085);font-size:.66rem;line-height:1.45}.settings-status{padding:5px 8px;border-radius:999px;background:#ecfdf3;color:#067647;font-size:.55rem;font-weight:900;white-space:nowrap}.settings-workspace :deep(.backup-panel){margin-top:0!important}.boundary{display:flex;gap:10px;margin-top:12px;padding:12px 14px;box-shadow:none;color:var(--kfe-muted-text,#667085);font-size:.66rem;line-height:1.5}.boundary strong{color:var(--kfe-ui-text,#101828)}.shield{width:25px;height:25px;display:grid;place-items:center;flex:0 0 25px;border-radius:8px;background:#ecfdf3;color:#067647;font-weight:950}@media(max-width:700px){.group-grid{grid-template-columns:1fr}.group-card{min-height:78px}.settings-layout{grid-template-columns:1fr}.settings-menu{padding:13px}.settings-menu button{min-height:55px}.admin-head{align-items:stretch;flex-direction:column}.admin-badge{align-self:flex-start}.workspace-head{align-items:flex-start}.record{flex-direction:column}.actions{justify-content:flex-end;width:100%}}@media(max-width:420px){.admin-page{padding:18px 13px 28px}.workspace,.settings-menu,.settings-workspace{padding:12px}.workspace-head h2,.settings-menu h2,.settings-heading h2{font-size:1rem}.primary{padding:0 10px}.record-tabs button{font-size:.58rem}.record{padding:11px}.settings-heading{flex-direction:column}}
</style>
