...
async function reconcileAutoSourcePayment({sourceType,sourceId,newCost,previousCost,sourceDate}) {
  if (!sourceId || !Number.isFinite(Number(newCost))) return
  const payments = live(performanceSnapshot.value?.settlements).filter(x => x.sourceId === sourceId && String(x.direction || '').toUpperCase() === 'OUT')
  const auto = payments.find(x => x.autoGenerated === true) || payments.find(x => Number(x.amount) === Number(previousCost) && String(x.settledOn || '').slice(0,10) === String(sourceDate || '').slice(0,10))
  if (!auto) return
  if (Number(newCost) <= 0) {
    await AdminService.remove('settlement', auto.id)
    return
  }
  await AdminService.save('settlement', {
    settlementType:'Payment',
    sourceType,
    sourceId,
    settledOn:auto.settledOn || sourceDate,
    amount:Number(newCost),
    autoGenerated:true
  }, auto.id)
}
async function save(values){clearMessages();loading.value=true;try{
  ...
  const previous=editing.value!==null&&isSource?records.value.find(x=>x.id===editing.value):null
  const previousCost=Number(previous?.values?.cost)
  const previousDate=selected.value==='maintenance'?previous?.values?.performedOn:previous?.values?.validFrom
  const saved=await AdminService.save(selected.value,payload,id)
  if(id===null&&isSource&&Number(payload.cost)>0){
    await AdminService.save('settlement',{settlementType:'Payment',sourceType,sourceId:saved.id,settledOn:selected.value==='maintenance'?payload.performedOn:payload.validFrom,amount:Number(payload.cost),autoGenerated:true})
  } else if(id!==null&&isSource&&Number.isFinite(previousCost)&&previousCost!==Number(payload.cost)){
    await reconcileAutoSourcePayment({sourceType,sourceId:saved.id,newCost:Number(payload.cost),previousCost,sourceDate:previousDate})
  }
