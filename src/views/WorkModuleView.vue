...
const cancelOffline = () => { endShiftOpen.value = false; fuelFormOpen.value = false; error.value = ''; notify('Still Online — End Shift cancelled.') }
const openFuelForm = () => { fuelFormOpen.value = !fuelFormOpen.value; if (fuelFormOpen.value) endShiftOpen.value=false; error.value=''; message.value='' }
...
<template>
  <div class="cockpit" :class="{ 'cockpit--offline': !store.isOnline, 'cockpit--online': store.isOnline && !store.isTripActive && !endShiftOpen, 'cockpit--trip': store.isTripActive, 'cockpit--end-shift': endShiftOpen, 'cockpit--fuel': fuelFormOpen }">
    ...
    <section v-if="fuelFormOpen" class="card gate fuel-form-card"><div class="section-heading"><div><small>CNG REFUELLING</small><h2>Refuelling</h2></div><span class="optional-badge">Anytime</span></div><p class="muted">ERP calculates quantity from amount ÷ price per kg. GPS and timestamp are captured automatically when available.</p><label>Odometer (km)<input v-model="fuelOdometer" type="number" min="0" inputmode="decimal"></label><label>Price per kg (₹)<input v-model="fuelPrice" type="number" min="0" step="0.01" inputmode="decimal"></label><label>Amount (₹)<input v-model="fuelAmount" type="number" min="0" step="0.01" inputmode="decimal"></label><div class="calculated"><span>ERP-calculated quantity</span><strong>{{fuelQuantity.toFixed(2)}} kg</strong></div><button class="primary" :disabled="fuelStore.saving" @click="saveFuel">{{fuelStore.saving?'Saving…':'OK'}}</button></section>
    <section v-if="!store.isTripActive && !store.isOnline" class="card gate"><h2>Start Shift</h2><p class="muted">The Online toggle opens this gate. The shift starts only after the odometer check succeeds.</p><label>Current odometer (km)<input v-model="startOdo" type="number" min="0" inputmode="decimal"></label><div v-if="store.firstKfeDay && gap.valid" class="gap gap-compact first-day-baseline"><div class="gap-head"><strong>First KFE operating day</strong><span>Historical gap excluded</span></div><div class="allocation-summary"><div><span>Business start</span><strong>{{store.businessStartBaseline?.businessStartOdometer ?? '—'}} km</strong></div><div><span>Current odometer</span><strong>{{startOdo || '—'}} km</strong></div></div><p class="muted">Any odometer difference from the Business Start Date to today is historical. It is not Personal KM or Dead KM and needs no allocation.</p></div><div v-else-if="gap.valid&&gapKm>0" class="gap gap-compact"><div class="gap-head"><strong>Odometer gap: {{gapKm}} km</strong><span>ERP calculated</span></div><div class="allocation-summary"><div><span>Personal KM</span><strong>{{gapPersonal}}</strong></div><div><span>Dead KM</span><strong>{{gapDead}}</strong></div></div><div class="allocation-actions"><button type="button" :class="{selected:gapCategory==='PERSONAL'}" @click="selectGapCategory('PERSONAL')">Personal KM</button><button type="button" :class="{selected:gapCategory==='DEAD'}" @click="selectGapCategory('DEAD')">Dead KM</button></div><div class="gap-status"><span v-if="gapCategory">Allocated {{allocatedGap}} / {{gapKm}} km to {{gapCategory==='PERSONAL'?'Personal KM':'Dead KM'}}</span><span v-else>Choose one category — entire gap will be assigned.</span><button v-if="gapCategory && !gapConfirmed" class="primary gap-confirm" type="button" @click="confirmGapAllocation">OK — Confirm allocation</button><span v-else-if="gapConfirmed" class="confirmed-text">✓ Allocation confirmed</span></div><div v-if="gapCategory==='PERSONAL'" class="two gap-expenses"><label>Personal toll<input v-model="personalToll" type="number" min="0" step="0.01" placeholder="₹ 0"></label><label>Personal parking<input v-model="personalParking" type="number" min="0" step="0.01" placeholder="₹ 0"></label></div></div><p v-else-if="gap.valid" class="muted">No odometer gap.</p><p v-else-if="startOdo" class="error-text">{{gap.reason}}</p></section>
