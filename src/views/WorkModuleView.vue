...
const selectGapCategory = category => {
  if (!gap.value.valid || gapKm.value <= 0) return
  gapCategory.value = category
  if (category !== 'PERSONAL') { personalToll.value = ''; personalParking.value = '' }
}
const goOnline = async () => {
  const needsGap = gap.value.valid && gapKm.value > 0
  const result = await store.startShift(startOdo.value, needsGap ? { category: gapCategory.value, personalToll: personalToll.value, personalParking: personalParking.value } : null)
  if (result.requiresGapAllocation) return fail(`Choose Personal KM or Dead KM to allocate the full ${result.gapKm} km before going Online.`)
  if (!result.ok) return fail(result.reason)
  startOdo.value = ''; gapCategory.value = null; personalToll.value = ''; personalParking.value = ''; await refreshTarget(); notify('Online.')
}
const goOffline = async (confirmLargeDistance = false) => {
  ...
  const result = await store.endShift({ closingOdometer: closingOdo.value, revenue: shiftRevenue.value, toll: toll.value, parking: parking.value, tollParkingRevenueTreatment: tollTreatment.value, trips, confirmLargeDistance })
  ...
  if (!result.ok) return fail(result.reason)
  closingOdo.value = ''; shiftRevenue.value = ''; toll.value = ''; parking.value = ''; tollTreatment.value = 'NONE'; reviewTrips.value = false; await refreshTarget(); notify('Offline.')
}
...
onMounted(async () => {
  await store.initialize(); await fuelStore.refresh(); await refreshTarget()
  startOdo.value = store.lastKnownOdometer ?? ''; selectedOperator.value = store.defaultOperator
  unsubscribeTarget = DriverTargetService.subscribeDataChanges(() => { void refreshTarget() })
...
<template>
  <div class="cockpit">
    ...
    <section class="target-card"><small>TODAY'S TARGET</small><strong>{{ targetText }}</strong></section>
