...
const form = ref({ operator: '', tripKm: '', revenue: '', cancelReason: '' })
...
const openEdit = trip => { editing.value=trip; form.value={operator:trip.operator||'',tripKm:trip.tripKm??'',revenue:trip.revenue??'',cancelReason:trip.cancelReason||'DRIVER_MISTAKE'} }
...
const timelineFieldValue = field => field === 'tripKm' ? form.value.tripKm : field === 'revenue' ? form.value.revenue : field === 'odometer' ? fuelForm.value.odometer : field === 'pricePerKg' ? fuelForm.value.pricePerKg : field === 'amount' ? fuelForm.value.amount : ''
const setTimelineFieldValue = (field,value) => { if(field==='tripKm') form.value.tripKm=value; else if(field==='revenue') form.value.revenue=value; else if(field==='odometer') fuelForm.value.odometer=value; else if(field==='pricePerKg') fuelForm.value.pricePerKg=value; else if(field==='amount') fuelForm.value.amount=value }
...
const save = async () => { closeTimelineKeypad(); saving.value=true; error.value=''; try { const r=await WorkService.updateTrip({id:editing.value.id,operator:form.value.operator,tripKm:form.value.tripKm,revenue:form.value.revenue,cancelReason:form.value.cancelReason}); if(!r?.ok) throw new Error(r?.reason||'Trip update failed.'); editing.value=null; await load() } catch(e) { error.value=e?.message||'Trip update failed.' } finally { saving.value=false } }
  ...
  <section class="timeline">
    ...
    <template v-else-if="period==='day' && data">
      ...
      <div v-for="event in data.events" :key="`${event.type}-${event.id}`">
        <article v-if="event.type==='TRIP'" class="event" :class="{ 'event--cancelled': event.record.status==='CANCELLED' }"><time>{{time(event.record.tripStartAt)}}</time><div><div class="top"><strong>{{event.record.operator}}</strong><small>{{event.record.status==='CANCELLED' ? 'Cancelled' : '✓ Completed'}}</small></div><div class="route">{{route(event.record)}}</div><div class="meta">{{event.record.status==='CANCELLED' ? (event.record.cancelledRevenue==null||event.record.cancelledRevenue===''?'Cancellation fee —':'Cancellation fee '+money(event.record.cancelledRevenue)) : (event.record.revenue==null||event.record.revenue===''?'Fare —':money(event.record.revenue))}} · {{km(event.record.tripKm)}} · {{duration(event.record)}}</div><small v-if="event.record.status==='CANCELLED' && event.record.cancelReason" class="cancelled-reason">{{event.record.cancelReason}}</small></div><button class="edit" @click="openEdit(event.record)" aria-label="Edit trip">✎</button></article>
    ...
    <Teleport to="body">
      ...
      <div v-if="timelineKeypadVisible" class="kfe-number-pad timeline-number-pad" aria-label="KFE number pad">
        <div class="number-pad-display"><span>{{timelineActiveField==='tripKm'?'DISTANCE':timelineActiveField==='revenue'?'FARE':timelineActiveField==='odometer'?'ODOMETER':timelineActiveField==='pricePerKg'?'PRICE / KG':'AMOUNT'}}</span><strong>{{timelineActiveField==='revenue'||timelineActiveField==='pricePerKg'||timelineActiveField==='amount'?'₹':''}}{{timelineFieldValue(timelineActiveField)||'0'}}<small v-if="timelineActiveField==='tripKm'||timelineActiveField==='odometer'"> km</small><small v-else-if="timelineActiveField==='pricePerKg'"> / kg</small></strong></div>
        ...
        <button class="number-pad-done" type="button" @click="closeTimelineKeypad">DONE</button>
      </div>
    </Teleport>
  </section>
</template>
