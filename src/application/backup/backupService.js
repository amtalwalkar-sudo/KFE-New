...
export const restoreBackup = async input => {
  ...
  await backupRepository.restoreCanonicalSnapshot(backup.stores)
...
export const registerCloudBackupProvider = provider => {
  if (!provider || typeof provider.upload !== 'function' || typeof provider.download !== 'function') throw new Error('Cloud backup provider must expose upload() and download().')
