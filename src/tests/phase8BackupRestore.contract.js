...
legacy.source.dbVersion = 8
legacy.stores = Object.fromEntries(Object.entries(legacy.stores).filter(([name]) => name !== 'audit_history'))
const migrated = BackupService.readBackupText(BackupService.serializeBackup({ ...legacy, formatVersion: 1 }))
assert.equal(migrated.formatVersion, BACKUP_FORMAT_VERSION)
assert.equal(migrated.source.dbVersion, 9)
assert.deepEqual(migrated.stores.audit_history, [])
...
assert.throws(() => BackupService.validateBackup({ ...record, source: { dbName: 'other', dbVersion: 9 } }), /source does not match/)
