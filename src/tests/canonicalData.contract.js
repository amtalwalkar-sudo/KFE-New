...
assert.match(canonicalDoc, /Provenance records where a value came from/); assert.match(canonicalDoc, /Authority is separate from provenance/); assert.match(shiftTripSource, /status !== 'ACTIVE'/); assert.match(shiftTripSource, /transitionTrip\(record, status, data\)/); assert.match(adminSource, /record\.deletedAt = now/); assert.match(adminSource, /record\.deleted = true/); assert.doesNotMatch(adminSource, /objectStore\([^)]*\)\.delete\(id\)/); assert.doesNotMatch(shiftTripSource, /objectStore\([^)]*\)\.delete\(id\)/); assert.doesNotMatch(fuelSource, /objectStore\([^)]*\)\.delete\(id\)/)
