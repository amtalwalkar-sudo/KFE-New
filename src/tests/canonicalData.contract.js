...
assert.match(shiftTripSource, /shift\.revenue = Number\(normalized\.revenue \|\| 0\)/); assert.match(shiftTripSource, /revenueAuthority = 'SUPPORTING_ONLY'/); assert.match(financeAdapterSource, /annualInterestRatePercent/); assert.doesNotMatch(financeAdapterSource, /annualInterestRate\s*:\s*10/); assert.match(financeAdapterSource, /status \|\| ''\)\.toUpperCase\(\) === 'ACTIVE'/); assert.match(adminSource, /LOAN_CONTRACT_FIELDS/); assert.match(adminSource, /AdminRepository\.correctLoan|correctLoan\(/); assert.match(adminSource, /action: 'CORRECTION'/)
...
assert.doesNotMatch(indexedDBSource, /createSimpleStore\(db, 'driver_collected_data'/); assert.match(indexedDBSource, /deleteObjectStore\('driver_collected_data'\)/); assert.equal(CANONICAL_BACKUP_STORES.includes('driver_collected_data'), false); assert.equal(BACKUP_FORMAT_VERSION, 3); assert.match(backupSource, /formatVersion === 2/); assert.match(backupSource, /driver_collected_data: _removed/); assert.match(canonicalDoc, /`driver_collected_data` is not part of the current canonical model/); assert.match(canonicalDoc, /database migration from version 9 to version 10 explicitly removes the obsolete store/); assert.match(canonicalDoc, /Backup format version 3/)
const legacyV2Stores = Object.fromEntries([...CANONICAL_BACKUP_STORES.slice(0, 11).map(store => [store, []]), ['driver_collected_data', []], ...CANONICAL_BACKUP_STORES.slice(11).map(store => [store, []])]); const migratedBackup = validateBackup({ format: 'KFE_BACKUP', formatVersion: 2, source: { dbName: 'kanishka_kfe_canonical_db', dbVersion: 9 }, exportedAt: new Date().toISOString(), stores: legacyV2Stores }); assert.equal(migratedBackup.formatVersion, 3); assert.equal('driver_collected_data' in migratedBackup.stores, false)
