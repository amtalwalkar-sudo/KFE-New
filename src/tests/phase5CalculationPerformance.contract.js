...
assert.equal(service.revenue,1000); assert.equal(service.vehicleKm,200); assert.equal(service.businessKm,150); assert.equal(service.deadKm,50)
assert.equal(service.fuelCost,2200); assert.equal(service.toll,100); assert.equal(service.parking,50); assert.equal(service.actualMaintenance,300)
assert.equal(service.runningCost,2650); assert.equal(service.operatingProfit,-1650); assert.equal(service.breakEvenRevenue,service.monthlyBreakEvenRevenue)
assert.equal(service.target,service.driverTarget); assert.ok(Number.isFinite(service.breakEvenRevenue)); assert.ok(Number.isFinite(service.driverTarget))
assert.ok(Math.abs(engine.monthlyBreakEvenRevenue-service.monthlyBreakEvenRevenue)<1e-12)
const mr=istMonthRange(range.to); const from=mr?.from||range.from; const to=mr?new Date(Math.min(mr.to.getTime(),range.to.getTime())):range.to
const args={trips:snapshot.trips,shifts:snapshot.shifts,driverTargets:snapshot.driverTargets,from,to,applicableBreakEven:service.monthlyBreakEvenRevenue,historicalBreakEvenForDay:({day})=>{assert.ok(istMonthRange(day));return service.monthlyBreakEvenRevenue}}
const target=deriveRollingDriverTarget(args)
assert.ok(Math.abs(service.driverTarget-target.currentDailyTarget)<1e-12); assert.equal(service.driverTargetRemainingEligibleDays,target.remainingEligibleDays); assert.ok(Math.abs(service.driverTargetAllocatedBeforeCurrentDay-target.targetAllocatedBeforeCurrentDay)<1e-12); assert.ok(Math.abs(service.driverTargetRemainingObligation-target.remainingObligation)<1e-12)
...
for(const key of ['revenue','vehicleKm','businessKm','deadKm','fuelCost','toll','parking','actualMaintenance','runningCost','operatingProfit','breakEvenRevenue','monthlyBreakEvenRevenue']) assert.equal(higher[key],service[key],`Driver Target input changed actual metric: ${key}`)
...
assert.equal(missing.breakEvenRevenue,null); assert.equal(missing.monthlyBreakEvenRevenue,null); assert.equal(missing.driverTarget,null); assert.equal(missing.driverTargetAvailable,false)
