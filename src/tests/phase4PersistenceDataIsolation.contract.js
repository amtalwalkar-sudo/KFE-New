...
assert(db.includes('const CANONICAL_DB_VERSION = 13'), 'Canonical schema version must be v12 after historical snapshots')
assert(db.includes('const SYNTHETIC_DB_VERSION = 12'), 'Synthetic schema version must be explicit')
...
assert(backup.includes("source: { dbName: 'kanishka_kfe_canonical_db', dbVersion: 13 }"), 'Backups must identify canonical source DB')
