...
for (const suite of suites) {
  ...
  const result = spawnSync(process.execPath, [`src/tests/${suite}`], {
    encoding: 'utf8',
...
for (const failure of failures) {
  process.stdout.write(`- ${failure.suite} (exit=${failure.status ?? 'null'}, signal=${failure.signal ?? 'none'})\n`)
}

if (failures.length > 0) {
  process.stdout.write('\n=== REQUIRED NEXT STEP ===\n')
  process.stdout.write('Inspect all failures together and group them by shared root cause before changing implementation or tests.\n')
  process.exitCode = 1
} else {
